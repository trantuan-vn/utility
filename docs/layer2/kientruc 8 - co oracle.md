...
graph TD
  ...
  EX3[Oracle Provider A]:::ext --> OCA
  EX4[Oracle Provider B]:::ext --> OCB
  EX5[Oracle Provider C]:::ext --> OCC
  EX6[CEX / On-ramp]:::ext --> E1
  EX7[L3 Rollup-as-a-Service]:::ext --> F1

  %% === Oracle Subsystem ===
  subgraph ORACLE[Multi-Oracle Network]
    OCA --> OA1[Oracle Aggregator]:::oracle
    OCB --> OA1
    OCC --> OA1
    OA1 --> OV1[Oracle Signature Verifier]:::oracle
    OV1 --> C1
    OV1 --> DP1
  end
  ...
  subgraph L2[TuấnChain - ZK Stack - Layer 2 Rollup]
    ...
    C5 --> M1[MEV Guard - Private Relay Sender]:::mev
    M1 --> M2[Stealth Relayer / Batch Submitter]:::mev
  end

  %% === L1 Ethereum ===
  subgraph L1[Ethereum - Layer 1]
    M2 --> D1[Verifier Contract - on Ethereum]:::eth
  ...
  classDef mev fill:#fce4ec,stroke:#c2185b,stroke-width:2px,font-weight:bold;
  classDef oracle fill:#fff0db,stroke:#cc7a00,stroke-width:1.5px;
